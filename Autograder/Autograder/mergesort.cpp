...
int numComparisonsMergesort = 0; // global variable to count number of comparisons made
int numMemAccessesMergesort = 0; // global variable to count number of memory accesses made
...
void Merge(std::vector<int> *numbers, int i, int j, int k)
{
   ...
   while (leftPos <= j && rightPos <= k)
   {
      ...
      if ((*numbers)[leftPos] < (*numbers)[rightPos])
      {
         ...
         numMemAccessesMergesort+=2; //increment numMemAccessMergsort by 2
         ++leftPos;
      }
      else
      {
         mergedNumbers[mergePos] = (*numbers)[rightPos];
         numMemAccessesMergesort += 2; // Incrementing the numAcceeMergesort by 2 2
   ...
   {
      mergedNumbers[mergePos] = (*numbers)[leftPos];
      numMemAccessesMergesort+=2; //Increment numAccessMergsort by 2
      ++leftPos;
      ++mergePos;
   }
