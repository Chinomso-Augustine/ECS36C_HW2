...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        //Part3: Checking for consecutive inversion

        for (auto items = data.begin(); items !=data.end(); ++items){// start from the beginning and don't stop until we reach the end of data
        { 

            string key =items.key();
            json value = items.value(); 
            ...
            for (int i = 0; i < value.size() - 1; i++) { // looping though all key values pair starting at i = counter =0 to size of the value
  ...
  //metadata to add back to result
