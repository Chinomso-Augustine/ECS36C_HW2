...
int main(int argc, char **argv)
{
    ...
    {
        cerr << "Error: Cannot open file " << firstFile << endl;
        return 1;
    }

    json file1Contents, file2Contents;
    ...
    { // Loops through each sample in both Json files.
        ...
        if (!firstFileArray.is_array() || !secondFileArray.is_array())
        {
            continue;
        }
        ...
        int minSize = min(firstFileArray.size(), secondFileArray.size());

        for (int pos = 0; pos < minSize; pos++)
        {
            if (!firstFileArray[pos].is_number() || !secondFileArray[pos].is_number())
            {
                mismatchList[to_string(pos)] = json::array({firstFileArray[pos], secondFileArray[pos]});
                anyMismatch = true;
                continue;
            }
            ...
            if (numFromFile1 != numFromFile2)
            {
                mismatchList[to_string(pos)] = json::array({numFromFile1, numFromFile2});
                anyMismatch = true;
            }
        }
