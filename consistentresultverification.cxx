...
int main(int argc, char **argv)
{
    ...
    int totalNumSamples = metadatagroup1["numSamples"].get<int>();
    ...
    { // Loops through each sample in both Json files.
        ...
        if (!file1Contents.count(SampName) || !file2Contents.count(SampName) ||
            !file1Contents[SampName].is_array() || !file2Contents[SampName].is_array())
        {
            cerr << "Warning: " << SampName << " missing in one of the files." << endl;
            continue;
        }

        json_file firstFileArray = file1Contents[SampName];  // Creating array for first file/accessing data from first file.
        json_file secondFileArray = file2Contents[SampName]; // Creating array for second file/accessing data from second file.
        ...
        size_t arraySize1 = firstFileArray.size();
        size_t arraySize2 = secondFileArray.size();
        size_t minArraySize = std::min(arraySize1, arraySize2);

        for (size_t pos = 0; pos < minArraySize; pos++)
        {                                             // Comparing each number in the arrays
            ...
            if (numFirstFile != numSecondFile)
            {   
                string posString = to_string(pos + 1);                                                    // Checking to see if the value at the position in the first sample array is the same as value in the second array sample
